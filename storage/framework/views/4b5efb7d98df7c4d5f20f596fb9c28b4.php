<div 
    x-data="{ 
        ...
        selectNext() {
            if (this.selectedIndex < <?php echo e(count($results)); ?> - 1) {
                this.selectedIndex++;
            }
        },
        selectPrev() {
                ...
                <ul x-ref="resultsList" class="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800 dark:text-gray-200" id="options" role="listbox">
                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $results; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $result): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li 
                            ...
                            :class="{ 'bg-indigo-600 text-white': selectedIndex === <?php echo e($index); ?>, 'hover:bg-gray-100 dark:hover:bg-gray-700': selectedIndex !== <?php echo e($index); ?> }"
                            role="option" 
                            tabindex="-1"
                            @click="window.location.href = '<?php echo e($result['url']); ?>'"
                            @mouseover="selectedIndex = <?php echo e($index); ?>"
                        >
                            <div class="flex items-center">
                                <!-- Icon logic based on type -->
                                <div class="flex-shrink-0 mr-3">
                                    <!--[if BLOCK]><![endif]--><?php if($result['icon'] === 'home'): ?>
                                        <i class="fas fa-home"></i>
                                    <?php elseif($result['icon'] === 'users' || $result['icon'] === 'user'): ?>
                                        <i class="fas fa-user"></i>
                                    <?php elseif($result['icon'] === 'calendar'): ?>
                                        <i class="fas fa-calendar"></i>
                                    <?php elseif($result['icon'] === 'dumbbell'): ?>
                                        <i class="fas fa-dumbbell"></i>
                                    <?php elseif($result['icon'] === 'cube'): ?>
                                        <i class="fas fa-cube"></i>
                                    <?php else: ?>
                                        <i class="fas fa-link"></i>
                                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                </div>
                                <span class="flex-auto truncate"><?php echo e($result['title']); ?></span>
                                <span class="ml-3 flex-none text-xs text-gray-400" :class="{ 'text-indigo-200': selectedIndex === <?php echo e($index); ?> }">
...
<?php /**PATH C:\xampp\htdocs\aria-training\resources\views/livewire/components/command-palette.blade.php ENDPATH**/ ?>