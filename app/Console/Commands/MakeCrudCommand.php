...
";
    ...
    protected function getFormProperties($fields)
    {
        ...
        foreach ($this->parseFields($fields) as $field) {
            $phpType = match ($field['type']) {
                'integer' => 'int', 'boolean' => 'bool', default => 'string'
            };
            $default = match ($field['type']) {
                'integer' => '0', 'boolean' => 'false', default => "''"
            };
    ...
    protected function getFormRules($fields, $modelName)
    {
        ...
        foreach ($this->parseFields($fields) as $field) {
            $rulesList = [];
            if (! in_array('nullable', $field['rules'])) {
                $rulesList[] = "'required'";
            }
            $rulesList[] = match ($field['type']) {
                'integer' => "'integer'", 'boolean' => "'boolean'", default => "'string'"
            };
            if (in_array($field['type'], ['string', 'text'])) {
                $rulesList[] = "'max:255'";
            }
            if (in_array('unique', $field['rules'])) {
                $rulesList[] = "ValidationRule::unique('{$table}')->ignore(\$this->model?->id)";
            }
    ...
    protected function getViewColumns($fields)
    {
        return $this->parseFields($fields);
    }

    protected function getViewFormFields($fields, $modelName)
    {
        return $this->parseFields($fields);
    }
